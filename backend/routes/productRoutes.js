...
//     }
//   } catch (e) {
//     console.error('Error clearing product cache:', e.message);
//   }
// };
...
// // GET /api/products (Ultra-fast Storefront Endpoint with Minimal Payload)
// router.get('/', async (req, res) => {
//   const { category, search, page, limit, t } = req.query;
//   const isPaginated = page !== undefined || limit !== undefined;
//   const pageNum = Math.max(1, parseInt(page, 10) || 1);
...
//   if (apiCache && typeof apiCache.get === 'function' && !t) {
//     const cached = apiCache.get(cacheKey);
//     if (cached) {
...
//       // Fast projection: Exclude heavy description for product cards
//       const [totalCount, prods] = await Promise.all([
//         (!search && isAllCategory)
//           ? Product.estimatedDocumentCount().maxTimeMS(2000).catch(() => Product.countDocuments(query))
//           : Product.countDocuments(query).maxTimeMS(3000),
//         Product.find(query)
//           .select('name price mrp image images category rating reviewsCount quantity remainingStock availableSizes createdAt')
//           .sort({ createdAt: -1 })
//           .skip(isPaginated ? skip : 0)
//           .limit(isPaginated ? limitNum : 0)
//           .lean()
...
//       res.setHeader('X-Total-Count', totalCount || 0);
//       res.setHeader('Access-Control-Expose-Headers', 'X-Total-Count');
//       if (apiCache && typeof apiCache.set === 'function') {
//         apiCache.set(cacheKey, responseData, 60); // 60s memory cache
//       }
//       return res.status(200).json(responseData);
...
//     clearProductCache();
//     if (isMongoConnected()) {
//       await Product.findByIdAndDelete(id).maxTimeMS(3000);
//     } else {
//       memoryProducts = memoryProducts.filter(p => p._id !== id);
//     }
//     const reqIo = req.app.get('io');
...
// GET /api/products (Ultra-fast Storefront Endpoint with Real DB Description)
router.get('/', async (req, res) => {
  const { category, search, page, limit, t } = req.query;
  const isPaginated = page !== undefined || limit !== undefined;
  const pageNum = Math.max(1, parseInt(page, 10) || 1);
  ...
  try {
    ...
    if (isMongoConnected()) {
      ...
      // Included 'description' in lean projection for zero-latency instant details
      const [totalCount, prods] = await Promise.all([
        (!search && isAllCategory)
          ? Product.estimatedDocumentCount().maxTimeMS(2000).catch(() => Product.countDocuments(query))
          : Product.countDocuments(query).maxTimeMS(3000),
        Product.find(query)
          .select('name price mrp image images category description rating reviewsCount quantity remainingStock availableSizes createdAt')
          .sort({ createdAt: -1 })
          .skip(isPaginated ? skip : 0)
          .limit(isPaginated ? limitNum : 0)
          .lean()
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (isMongoConnected()) {
      ...
      if (!prod) return res.status(404).json({ message: 'Product not found' });
      return res.json(sanitizeProduct(prod));
    } else {
      const prod = memoryProducts.find(p => p._id === id);
      if (!prod) return res.status(404).json({ message: 'Product not found' });
      return res.json(sanitizeProduct(prod));
  ...
  try {
    ...
    if (isMongoConnected()) {
      ...
      if (prod) {
        ...
        prod.images = processedImages;
        prod.image = processedImages[0];
        prod.description = description ? String(description).trim() : '';
        prod.availableSizes = Array.isArray(parsedSizes) ? parsedSizes : [];
