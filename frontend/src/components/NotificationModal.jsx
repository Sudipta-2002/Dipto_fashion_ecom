        ...
        {selectedNotification ? (
          ...
          <div style={{ padding: '1.5rem 1.25rem', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
              {/* CATEGORY & TIME BADGE */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: '800', padding: '4px 12px', borderRadius: '20px', background: getCategoryBadge(selectedNotification.category || selectedNotification.type).bg, color: getCategoryBadge(selectedNotification.category || selectedNotification.type).color, border: '1px solid rgba(0,0,0,0.08)', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                  {getCategoryBadge(selectedNotification.category || selectedNotification.type).icon} {getCategoryBadge(selectedNotification.category || selectedNotification.type).label}
                </span>
                <span style={{ fontSize: '0.78rem', color: '#94a3b8', fontWeight: '600' }}>
                  {getTimeAgo(selectedNotification.createdAt)}
                </span>
