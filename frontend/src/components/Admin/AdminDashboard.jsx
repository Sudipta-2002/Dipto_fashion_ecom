...
const AdminDashboard = ({ onNavigateToOrders, onNavigateToReturns, realtimeOrderUpdate }) => {
  ...
  useEffect(() => {
    if (realtimeOrderUpdate) {
      const amt = Number(realtimeOrderUpdate.totalAmount || realtimeOrderUpdate.amount || 0);
      setStats((prev) => ({
        ...prev,
        totalOrders: (prev.totalOrders || 0) + 1,
        pendingOrdersCount: (prev.pendingOrdersCount || 0) + 1,
        todaySales: (prev.todaySales || 0) + amt,
        totalSales: (prev.totalSales || 0) + amt
      }));
    }
  }, [realtimeOrderUpdate]);
    ...
    <div>
      ...
      {stats.pendingReturnsCount > 0 && (
        <div style={{
          ...
          <button
            className="btn-primary"
            style={{ background: '#e11d48', borderColor: '#be123c', padding: '0.65rem 1.25rem', whiteSpace: 'nowrap', cursor: 'pointer' }}
            onClick={onNavigateToReturns || onNavigateToOrders}
          >
            <span>Manage Returns ({stats.pendingReturnsCount})</span>
            <ArrowRight size={16} />
      ...
      {stats.pendingOrdersCount > 0 && (
        <div style={{
          ...
          <button
            className="btn-primary"
            style={{ background: '#f97316', borderColor: '#ea580c', padding: '0.65rem 1.25rem', whiteSpace: 'nowrap', cursor: 'pointer' }}
            onClick={onNavigateToOrders}
          >
            <span>Review Orders ({stats.pendingOrdersCount})</span>
            <ArrowRight size={16} />
      ...
      <div style={{ background: 'white', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
        <form onSubmit={handleFilter} style={{ display: 'flex', alignItems: 'flex-end', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <button type="submit" className="btn-primary" style={{ padding: '0.55rem 1.25rem', cursor: 'pointer' }}>
            <Filter size={16} /> Filter Date Range
          </button>
          {(startDate || endDate) && (
            <button
              type="button"
              className="btn-outline"
              onClick={() => { setStartDate(''); setEndDate(''); setLoading(true); setTimeout(() => fetchAnalytics(true), 100); }}
      ...
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        <div className="stat-card">
          ...
          <div className="value">₹{(stats.todaySales || 0).toLocaleString('en-IN')}</div>
          <p style={{ fontSize: '0.75rem', color: '#16a34a', fontWeight: '600', marginTop: '4px' }}>
            ✔ Accepted Orders Today
          </p>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="value">₹{(stats.monthlySales || 0).toLocaleString('en-IN')}</div>
          <p style={{ fontSize: '0.75rem', color: '#16a34a', fontWeight: '600', marginTop: '4px' }}>
            ✔ Accepted Orders This Month
          </p>
        </div>
        ...
        <div className="stat-card" style={{ border: '1.5px solid #fecdd3', background: '#fff1f2' }}>
          ...
          <div className="value" style={{ color: '#e11d48' }}>
            {stats.dailyReturnQty || 0} Pcs <span style={{ fontSize: '0.9rem', opacity: 0.8 }}>(₹{(stats.dailyReturnAmount || 0).toLocaleString('en-IN')})</span>
          </div>
          <p style={{ fontSize: '0.75rem', color: '#be123c', fontWeight: '600', marginTop: '4px' }}>
            ↩ Daily Returned Quantity & Amount
          </p>
        ...
        <div className="stat-card" style={{ border: '1.5px solid #fecdd3', background: '#fff1f2' }}>
          ...
          <div className="value" style={{ color: '#e11d48' }}>
            {stats.monthlyReturnQty || 0} Pcs <span style={{ fontSize: '0.9rem', opacity: 0.8 }}>(₹{(stats.monthlyReturnAmount || 0).toLocaleString('en-IN')})</span>
          </div>
          <p style={{ fontSize: '0.75rem', color: '#be123c', fontWeight: '600', marginTop: '4px' }}>
            ↩ Monthly Total Returns & Refunds
          </p>
          ...
          {stats.chartData && stats.chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" />
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <Tooltip formatter={(val) => [`₹${val?.toLocaleString('en-IN')}`, 'Sales']} />
                <Area type="monotone" dataKey="sales" stroke="#c026d3" strokeWidth={3} fillOpacity={1} fill="url(#salesGradient)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
          ...
          {stats.returnChartData && stats.returnChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.returnChartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" />
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                <Tooltip formatter={(val, name) => [name === 'returnAmount' ? `₹${val?.toLocaleString('en-IN')}` : `${val} Items`, name === 'returnAmount' ? 'Refund Amount' : 'Returned Quantity']} />
                <Bar dataKey="returnQty" fill="#e11d48" name="Returned Quantity" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
