...
import { RotateCcw, CheckCircle2, Landmark, RefreshCw, Calendar, AlertCircle, Ban, CreditCard, Smartphone, Building2 } from 'lucide-react';
import TableSkeleton from '../Skeletons/TableSkeleton';
import { API_URL } from '../../api';
import { fetchWithCache, clearCache } from '../../utils/cache';
...
const AdminReturns = () => {
  ...
  const handleUpdateReturnStatus = async (orderId, newStatus) => {
    let previousRequests = allRequests;
    setAllRequests((prev) =>
      prev.map((o) =>
        (o._id === orderId || o.orderId === orderId) ? { ...o, status: newStatus } : o
      )
              ...
              <tr key={o._id || o.orderId}>
                <td>
                  ...
                  <div style={{ fontSize: '0.73rem', color: '#94a3b8', marginTop: '2px' }}>
                    {new Date(
                      isCancellationType
                        ? (o.cancellationDetails?.requestedAt || o.updatedAt || o.createdAt)
                        : (o.returnDetails?.requestedAt || o.updatedAt || o.createdAt)
                    ).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </div>
                </td>
                ...
                <td>
                  {isCancellationType ? (
                    ...
                    <button
                      className="btn-outline"
                      style={{ padding: '0.32rem 0.6rem', fontSize: '0.78rem', cursor: 'pointer' }}
                      onClick={() => setSelectedDetails({ ...o, type: 'return' })}
                    >
                      <Landmark size={13} /> View Bank / UPI
                    </button>
  ...
  const renderGroup = (itemsList, title, subtitle, borderColor, bgColor, badgeBg, textColor) => (
    <div key={title} style={{ background: 'white', borderRadius: '14px', border: `1.5px solid ${borderColor}`, boxShadow: `0 4px 12px ${bgColor}22`, overflow: 'hidden' }}>
      <div style={{ background: bgColor, padding: '0.85rem 1.25rem', borderBottom: `1.5px solid ${borderColor}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: `${borderColor}44`, color: textColor, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Calendar size={18} />
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <button className="btn-outline" onClick={() => fetchRequests(true)} style={{ padding: '0.4rem 0.75rem', fontSize: '0.82rem', cursor: 'pointer' }}>
          <RefreshCw size={14} /> Refresh
        </button>
      </div>
            ...
            <div className="modal-body">
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '1rem', marginBottom: '1rem' }}>
                {selectedDetails.returnDetails?.upiId ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem' }}>
                    <div><strong>Account Holder:</strong> {selectedDetails.returnDetails?.accountHolder || 'N/A'}</div>
                    <div><strong>Bank Name:</strong> {selectedDetails.returnDetails?.bankName || 'N/A'}</div>
                    <div><strong>Account Number:</strong> {selectedDetails.returnDetails?.accountNumber || 'N/A'}</div>
                    <div><strong>IFSC Code:</strong> {selectedDetails.returnDetails?.ifscCode || 'N/A'}</div>
                  </div>
                )}
              </div>
              <div style={{ fontSize: '0.85rem', color: '#334155' }}>
