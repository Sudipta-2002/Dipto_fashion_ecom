...
const AdminBilling = () => {
  ...
  // Summary calculations
  const totalSoldAmount = filteredEntries
    .filter((e) => e.type === 'credit' || e.sign === '+')
    .reduce((sum, e) => sum + Math.abs(e.rawAmount || e.amount || 0), 0);

  const totalRefundAmount = filteredEntries
    .filter((e) => e.type === 'debit' || e.sign === '-')
    .reduce((sum, e) => sum + Math.abs(e.rawAmount || e.amount || 0), 0);
  ...
  const handleExportExcel = () => {
    ...
    const worksheetData = [
      ...
      [`Total Sold Sales (+): ₹${totalSoldAmount.toLocaleString('en-IN')}`, '', `Total Refunds (-): ₹${totalRefundAmount.toLocaleString('en-IN')}`, `NET TOTAL BILL: ₹${netTotalBill.toLocaleString('en-IN')}`],
      [],
      ['Date & Time', 'Order ID', 'Customer Email', 'UTR', 'Status', 'Transaction Amount']
    ];

    filteredEntries.forEach((item) => {
      const isCredit = item.type === 'credit' || item.sign === '+';
      const displayAmount = Math.abs(item.rawAmount || item.amount || 0);
      const formattedDate = new Date(item.date).toLocaleDateString('en-GB', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
            ...
            {filteredEntries.length === 0 ? (
              ...
              <div className="table-responsive">
                <table className="admin-table">
                  ...
                  <tbody>
                    {filteredEntries.map((entry) => {
                      const isCredit = entry.type === 'credit' || entry.sign === '+';
                      const displayAmount = Math.abs(entry.rawAmount || entry.amount || 0);
                      ...
                      return (
                        <tr key={entry.id}>
                          ...
                          {/* 6. Amount (+ for Shipped / - for Return) */}
                          <td
                            style={{
                              textAlign: 'right',
                              fontWeight: '900',
