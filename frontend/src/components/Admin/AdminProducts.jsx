...
import { Plus, Trash2, Edit, Upload, X, CheckCircle2, AlertCircle, Package } from 'lucide-react';
import { API_URL } from '../../api';
import { fetchWithCache, clearCache } from '../../utils/cache';
...
const AdminProducts = () => {
  ...
  const fetchProducts = useCallback(async (pageNum = 1, isAppend = false, forceRefresh = false) => {
    const cacheKey = `admin_products_page_${pageNum}`;

    if (pageNum === 1 && !isAppend && !forceRefresh) {
      const { data: cached } = await fetchWithCache(
        cacheKey,
        async () => {
          const res = await fetch(`${API_URL}/api/products?page=${pageNum}&limit=20`);
          const totalHeader = res.headers.get('X-Total-Count');
          const total = totalHeader ? parseInt(totalHeader, 10) : 0;
          const data = await res.json();
          const list = Array.isArray(data) ? data : (data?.products || []);
          return { data: list, total: total || data?.totalProducts || list.length };
        }
      );
      ...
      if (cached && cached.data) {
        ...
        setPage(1);
        setHasMore(cached.data.length === 20 && cached.total > cached.data.length);
        setLoading(false);
        return;
      }
    }
    ...
    try {
      const res = await fetch(`${API_URL}/api/products?page=${pageNum}&limit=20&t=${Date.now()}`);
      const totalHeader = res.headers.get('X-Total-Count');
      const total = totalHeader ? parseInt(totalHeader, 10) : 0;
      const data = await res.json();
      const list = Array.isArray(data) ? data : (data?.products || []);
      ...
      if (isAppend) {
        setProducts((prev) => {
          ...
          const newUnique = list.filter((p) => !existingIds.has(p._id || p.id));
      ...
      const totalNum = total || data?.totalProducts || list.length;
      setTotalCount(totalNum);
      setPage(pageNum);
      const currentLoaded = isAppend ? products.length + list.length : list.length;
      setHasMore(list.length === 20 && (totalNum ? currentLoaded < totalNum : true));
    } catch (e) {
      console.error('Error fetching products:', e);
    } finally {
      setLoading(false);
  ...
  const handleSaveProduct = async (e) => {
    ...
    if (editingProduct) {
      setProducts(prev => prev.map(p =>
        (p._id || p.id) === (editingProduct._id || editingProduct.id) ? { ...p, ...payload } : p
      ));
    } else {
      const tempProduct = { ...payload, _id: 'temp_' + Date.now(), images: uploadedImages };
      setProducts(prev => [tempProduct, ...prev]);
    ...
    const isEditing = !!editingProduct;
    const editingProductId = editingProduct ? (editingProduct._id || editingProduct.id) : null;
    resetForm();
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json();
        setProducts(previousProducts);
        showError(errData.message || 'Failed to save product');
      }
    } catch (e) {
      setProducts(previousProducts);
      showError('Network error saving product: ' + e.message);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: '800', margin: 0 }}>Product Inventory Management</h3>
          <span style={{ fontSize: '0.82rem', fontWeight: '700', color: '#64748b' }}>
            Loaded: {products.length} {totalCount ? `/ ${totalCount} Total Products` : 'Products'}
          </span>
        </div>
        <button
          className="btn-primary"
          ...
          <form onSubmit={handleSaveProduct}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
              ...
              <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                <label style={{ fontWeight: '800', color: '#c026d3', display: 'block', marginBottom: '0.4rem' }}>
                  📏 Available Product Sizes (Optional)
                </label>
                <div style={{ display: 'flex', gap: '0.65rem', flexWrap: 'wrap', background: '#fdf4ff', padding: '0.85rem', borderRadius: '8px', border: '1px solid #f5d0fe' }}>
                  {ALL_SIZES.map(s => {
                    const isSelected = selectedSizes.includes(s);
              ...
              <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                <label>Upload Product Images * (Min 1, Max 5 Images)</label>
                <div style={{ border: '2px dashed #cbd5e1', borderRadius: '8px', padding: '0.85rem', background: '#f8fafc', textAlign: 'center' }}>
                  <input
                    type="file"
                    accept="image/*"
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
              <button type="submit" className="btn-primary" disabled={formSubmitting} style={{ cursor: 'pointer' }}>
                {formSubmitting ? 'Saving Product...' : editingProduct ? 'Update Product' : 'Save Product'}
              </button>
              <button type="button" className="btn-outline" onClick={resetForm} style={{ cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </form>
      ...
      {loading ? (
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '3rem', textAlign: 'center', color: '#94a3b8', border: '1px solid #e2e8f0' }}>
          ...
          <h4>No Products Found in Inventory</h4>
        </div>
      ) : (
        <>
          <div className="table-responsive">
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <button
                            onClick={() => handleStartEdit(p)}
                            className="btn-outline"
                            style={{ padding: '0.35rem 0.6rem', fontSize: '0.8rem', color: '#0284c7', borderColor: '#38bdf8', cursor: 'pointer' }}
                            title="Edit Product"
                          >
                            <Edit size={14} /> Edit
                          </button>
                          <button
                            onClick={() => handleDeleteProduct(p._id || p.id)}
                            style={{ background: '#fef2f2', color: '#ef4444', border: '1px solid #fca5a5', padding: '0.35rem 0.6rem', borderRadius: '6px', cursor: 'pointer' }}
                            title="Delete Product"
                          >
                            <Trash2 size={14} />
                          </button>
          ...
          {/* Load More Button for Big Data Scale */}
          {hasMore && (
            <div style={{ textAlign: 'center', marginTop: '1.25rem' }}>
              ...
              >
                {loadingMore ? 'Loading More Products...' : `Load More Products (${products.length} of ${totalCount})`}
              </button>
            </div>
          )}
        </>
