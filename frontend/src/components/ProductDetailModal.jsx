...
const ProductDetailModal = ({
  ...
  const [isSizeChartOpen, setIsSizeChartOpen] = useState(false);
  const [isFullViewOpen, setIsFullViewOpen] = useState(false);
  const [isDetailsExpanded, setIsDetailsExpanded] = useState(false); // Closed by default
  const [isReviewsExpanded, setIsReviewsExpanded] = useState(false);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIsFullViewOpen(false);
      setSizeError(false);
      setIsDetailsExpanded(false); // Reset to closed on open
      setIsReviewsExpanded(false);
      setSelectedSize(sizesList.length === 1 ? sizesList[0] : '');
      ...
      <div className="product-detail-main-wrapper" style={{ maxWidth: '1280px', margin: '0 auto', width: '100%', padding: '2rem 1.5rem 4rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(320px, 480px) 1fr', gap: '3rem', alignItems: 'start' }} className="product-detail-layout">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* 1. ALL DETAILS & PRODUCT SPECIFICATIONS ACCORDION (Closed by default) */}
            <div
              style={{
                border: '1.5px solid #e2e8f0',
                borderRadius: '14px',
            ...
            {/* 2. RATINGS & VERIFIED CUSTOMER REVIEWS DROPDOWN ACCORDION (Closed by default) */}
            <div
              style={{
                border: '1.5px solid #e2e8f0',
                borderRadius: '14px',
