...
const AuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  // High-Speed Instant Click Handler
  const handleGoogleAuth = () => {
    setError('');
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || 'https://www.diptofashion.in/auth/google/callback';
    
    // Direct OAuth Popup Trigger for zero latency
  ...
  const validateForm = () => {
    ...
    if (mode === 'signup') {
      if (!formData.name.trim()) errors.name = 'Full Name is required';
      if (!formData.email.trim() || !emailRegex.test(formData.email.trim())) errors.email = 'Valid email is required';
      const cleanPhone = formData.phone.trim().replace(/\D/g, '');
      if (!cleanPhone || cleanPhone.length !== 10) errors.phone = '10-digit mobile required';
      if (!formData.password || formData.password.length < 8) errors.password = 'Min. 8 characters required';
      if (formData.password !== formData.confirmPassword) errors.confirmPassword = 'Passwords do not match';
    } else if (mode === 'login') {
      if (!formData.email.trim() || !emailRegex.test(formData.email.trim())) errors.email = 'Valid email required';
      if (!formData.password || formData.password.length < 8) errors.password = 'Min. 8 characters required';
    } else if (mode === 'forgot') {
      if (!formData.email.trim() || !emailRegex.test(formData.email.trim())) errors.email = 'Valid email required';
      if (!formData.newPassword || formData.newPassword.length < 8) errors.newPassword = 'Min. 8 characters required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = mode === 'signup' ? '/api/auth/send-otp' : mode === 'login' ? '/api/auth/login-check' : '/api/auth/forgot-password-check';
      const payload = mode === 'signup' ? { email: cleanEmail, type: 'signup' } : mode === 'login' ? { email: cleanEmail, password: formData.password } : { email: cleanEmail, newPassword: formData.newPassword };
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setError('');
    try {
      const endpoint = otpOrigin === 'signup' ? '/api/auth/verify-signup' : otpOrigin === 'login' ? '/api/auth/verify-login-otp' : '/api/auth/verify-reset-password';
      const payload = otpOrigin === 'signup' 
        ? { name: formData.name.trim(), email: otpTargetEmail, phone: formData.phone.trim(), password: formData.password, otp: otpCode }
        : otpOrigin === 'login'
        ? { email: otpTargetEmail, otp: otpCode }
        : { email: otpTargetEmail, newPassword: formData.newPassword, otp: otpCode };
  ...
  return (
    <div className="modal-overlay" onClick={onClose} style={{ backdropFilter: 'blur(6px)', zIndex: 300 }}>
      ...
      >
        {/* Left Branding Panel */}
        <div style={{ width: '40%', background: 'linear-gradient(160deg, #1e1b4b 0%, #701a75 100%)', padding: '2.25rem 1.75rem', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxSizing: 'border-box' }}>
          <div>
            <h2 style={{ fontSize: '1.55rem', fontWeight: '800', margin: 0 }}>
              {mode === 'otp' ? 'Verification' : mode === 'forgot' ? 'Reset Password' : mode === 'login' ? 'Login' : 'Sign Up'}
            </h2>
            <p style={{ fontSize: '0.85rem', color: '#f5d0fe', marginTop: '0.5rem' }}>
              {mode === 'login' ? 'Access your orders, wishlist, and profile.' : 'Get started with Dipto Fashion.'}
            </p>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontWeight: '800', fontSize: '1.2rem' }}>Dipto Fashion</div>
            <div style={{ fontSize: '0.72rem', color: '#f5d0fe', opacity: 0.85 }}>Premium Ethnic Collection</div>
          </div>
          <div style={{ fontSize: '0.72rem', color: '#e9d5ff', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <ShieldCheck size={16} /> 100% Secure Authentication
          </div>
        </div>

        {/* Right Form Panel */}
        <div style={{ width: '60%', padding: '2rem 1.75rem', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflowY: 'auto', boxSizing: 'border-box' }}>
          <button onClick={onClose} style={{ position: 'absolute', top: '16px', right: '16px', color: '#64748b', background: '#f1f5f9', border: 'none', borderRadius: '50%', width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <X size={18} />
          </button>

          <div>
            {error && <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', color: '#b91c1c', padding: '0.6rem', borderRadius: '8px', marginBottom: '0.8rem', fontSize: '0.82rem', fontWeight: '600' }}>⚠️ {error}</div>}
            {successMsg && <div style={{ background: '#f0fdf4', border: '1px solid #86efac', color: '#15803d', padding: '0.6rem', borderRadius: '8px', marginBottom: '0.8rem', fontSize: '0.82rem', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px' }}><CheckCircle2 size={16} /> {successMsg}</div>}

            {mode === 'otp' ? (
              <form onSubmit={handleVerifyOtp} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ textAlign: 'center' }}>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: '800' }}>Enter OTP Code</h3>
                  <p style={{ fontSize: '0.78rem', color: '#64748b' }}>Sent to <strong style={{ color: '#c026d3' }}>{otpTargetEmail}</strong></p>
                </div>
                <div style={{ display: 'flex', justifyContent: 'center', gap: '0.45rem' }}>
                  {otpValues.map((val, idx) => (
                    <input key={idx} ref={(el) => (otpInputRefs.current[idx] = el)} type="text" maxLength={1} value={val} onChange={(e) => handleOtpChange(idx, e.target.value)} onKeyDown={(e) => handleOtpKeyDown(idx, e)} style={{ width: '40px', height: '46px', textAlign: 'center', fontSize: '1.2rem', fontWeight: '800', borderRadius: '8px', border: val ? '2px solid #c026d3' : '1px solid #cbd5e1' }} />
                  ))}
                </div>
                <button type="submit" disabled={loading} style={{ height: '44px', background: '#c026d3', color: 'white', borderRadius: '8px', border: 'none', fontWeight: '800', cursor: 'pointer' }}>{loading ? 'VERIFYING...' : 'VERIFY & CONTINUE'}</button>
              </form>
            ) : mode === 'forgot' ? (
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                <div style={{ position: 'relative' }}>
                  <input type="email" name="email" placeholder="Registered Gmail" value={formData.email} onChange={handleChange} style={inputStyle(fieldErrors.email)} />
                  <Mail size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                </div>
                <div style={{ position: 'relative' }}>
                  <input type={showPassword ? 'text' : 'password'} name="newPassword" placeholder="New Password" value={formData.newPassword} onChange={handleChange} style={inputStyle(fieldErrors.newPassword)} />
                  <Lock size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                </div>
                <button type="submit" disabled={loading} style={{ height: '44px', background: '#c026d3', color: 'white', borderRadius: '8px', border: 'none', fontWeight: '800', cursor: 'pointer' }}>{loading ? 'SENDING OTP...' : 'RESET PASSWORD'}</button>
              </form>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                {mode === 'signup' && (
                  <div style={{ position: 'relative' }}>
                    <input type="text" name="name" placeholder="Full Name" value={formData.name} onChange={handleChange} style={inputStyle(fieldErrors.name)} />
                    <User size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  </div>
                )}
                <div style={{ position: 'relative' }}>
                  <input type="email" name="email" placeholder="Gmail Address" value={formData.email} onChange={handleChange} style={inputStyle(fieldErrors.email)} />
                  <Mail size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                </div>
                {mode === 'signup' && (
                  <div style={{ position: 'relative' }}>
                    <input type="text" name="phone" placeholder="10-digit Mobile Number" maxLength={10} value={formData.phone} onChange={handleChange} style={inputStyle(fieldErrors.phone)} />
                    <Phone size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  </div>
                )}
                <div style={{ position: 'relative' }}>
                  <input type={showPassword ? 'text' : 'password'} name="password" placeholder="Password (Min. 8 characters)" value={formData.password} onChange={handleChange} style={inputStyle(fieldErrors.password)} />
                  <Lock size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                ...
                {mode === 'login' && (
                  <div style={{ textAlign: 'right' }}>
                    <button type="button" onClick={() => { setMode('forgot'); resetForm(); setMode('forgot'); }} style={{ background: 'none', border: 'none', color: '#c026d3', fontSize: '0.75rem', fontWeight: '700', cursor: 'pointer' }}>Forgot Password?</button>
                  </div>
                )}
                {mode === 'signup' && (
                  <div style={{ position: 'relative' }}>
                    <input type={showConfirmPassword ? 'text' : 'password'} name="confirmPassword" placeholder="Confirm Password" value={formData.confirmPassword} onChange={handleChange} style={inputStyle(fieldErrors.confirmPassword)} />
                    <Lock size={18} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  </div>
                )}
                <button type="submit" disabled={loading} style={{ height: '44px', background: '#c026d3', color: 'white', borderRadius: '8px', border: 'none', fontWeight: '800', cursor: 'pointer', textTransform: 'uppercase' }}>
                ...
                <div style={{ margin: '0.2rem 0' }}>
                  ...
                  >
                    ...
                    <span>{googleLoading ? 'Connecting...' : mode === 'signup' ? 'Sign up with Google' : 'Sign in with Google'}</span>
                  </button>
                </div>
              </form>
            )}
          ...
          <div style={{ textAlign: 'center', marginTop: '0.8rem' }}>
            <button
              type="button"
              onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); resetForm(); setMode(mode === 'login' ? 'signup' : 'login'); }}
              style={{ background: '#fdf4ff', border: '1px solid #f5d0fe', color: '#c026d3', padding: '0.55rem 1rem', borderRadius: '8px', fontWeight: '800', fontSize: '0.82rem', cursor: 'pointer', width: '100%' }}
            >
              {mode === 'login' ? 'New to Dipto Fashion? Create an account' : 'Existing User? Log in'}
            </button>
          </div>
        </div>
      </div>
