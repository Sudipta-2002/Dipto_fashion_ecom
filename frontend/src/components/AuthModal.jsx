...
//     setError('');
//     const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || 'https://www.diptofashion.in/auth/google/callback';
//     const authUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent('openid profile email')}&prompt=select_account`;
...
//                 {/* ORIGINAL GOOGLE BUTTON DESIGN (NOW LIGHTNING FAST) */}
//                 <div style={{ margin: '0.25rem 0 0 0' }}>
//                   <div style={{ display: 'flex', alignItems: 'center', margin: '0.65rem 0', gap: '0.75rem' }}>
//                     <div style={{ flex: 1, height: '1px', background: '#e2e8f0' }} />
//                     <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
...
const AuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  // Google Auth Success Listener (Instant zero-delay login like OTP)
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data && event.data.type === 'GOOGLE_AUTH_SUCCESS' && event.data.user) {
        setGoogleLoading(false);
        ...
        // Instant React state update & close modal
        if (onAuthSuccess) {
          onAuthSuccess(event.data.user);
        }
        if (onClose) {
