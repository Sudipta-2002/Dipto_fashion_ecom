...
import React, { useState, useEffect } from 'react';
import { CheckCircle, AlertTriangle, X, RefreshCw, ArrowLeft, Lock } from 'lucide-react';
import CheckoutProgressTracker from './CheckoutProgressTracker';
import { RAZORPAY_KEY_ID, apiFetch } from '../api';
import { loadRazorpayScript } from '../utils/loadRazorpay';
import { formatFullAddress } from '../utils/addressFormatter';
...
const PaymentModal = ({
  ...
  const [isQrRemoved, setIsQrRemoved] = useState(false);
  const [paymentInitiated, setPaymentInitiated] = useState(false);
  const [redirectCountdown, setRedirectCountdown] = useState(5); // 5-Second Auto Redirect Timer
  ...
  // Requirement: Auto-Redirect to Store Front Page after 5-Second Success Display
  useEffect(() => {
    let redirectTimer = null;
  ...
  // Automatic Web Redirection & Return Listener after Mobile UPI Payment
  useEffect(() => {
    const handleReturnToWeb = () => {
      if (isOpen && step === 'payment' && paymentInitiated) {
        setIsQrRemoved(true);
  ...
  // Safe HTTP Response Parsing Helper
  const parseResponseSafely = async (res) => {
    const contentType = res.headers.get('content-type') || '';
    if (contentType.includes('application/json')) {
      return await res.json();
  ...
  return (
    <div className="modal-overlay">
      ...
      >
        ...
        {step === 'success' && (
          /* ORDER SUCCESSFULLY PLACED PAGE (5-SECOND COUNTDOWN & AUTO-REDIRECT) */
          <div
            className="modal-body"
            style={{
              textAlign: 'center',
